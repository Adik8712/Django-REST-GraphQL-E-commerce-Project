...
# Импортируем переменную schema из модуля schema.py, 
# которая содержит определение схемы GraphQL.
...
class MyGraphQLView(GraphQLView):
    ...
    # чтобы добавить дополнительную логику перед выполнением запроса GraphQL. 
...
def graphql_view(request):
    # Создаем экземпляр CustomGraphQLView с 
    # передачей параметров graphiql=True и schema=schema.
    view = MyGraphQLView.as_view(graphiql=True, schema=schema)
    # Вызываем представление view, 
    # передавая объект request для обработки запроса GraphQL.
    return view(request)
